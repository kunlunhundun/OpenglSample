...
//                batch();
        ...
        int inputBufferIndex = decoder.dequeueInputBuffer(ConstAV.BUFFER_TIME_OUT);
...
//                batch();
            ...
            while (true) {
                int outputBufferIndex = decoder.dequeueOutputBuffer(bufferInfo, ConstAV.BUFFER_TIME_OUT);
    ...
    private void initDecoder(Surface surface) {
        try {
            ...
            initTrack();
            decoder = MediaCodec.createDecoderByType(ConstAV.VideoParams.MIME_TYPE);
        } catch (IOException e) {
            return;
        }
    ...
    private void initTrack() {
        ...
        for (int i = 0; i < numTracks; i++) {
            ...
            if (mime.equals(ConstAV.VideoParams.MIME_TYPE)) {
                videoTrack = i;
