...
import com.kunlun.douyindemo.com.common.util.ThreadUtil;
...
public class AudioRecorder implements Recorder<AudioConfig>, Runnable {
    ...
    public void configure(AudioConfig configuration) {
        ...
        int bufferSize = AudioRecord.getMinBufferSize(
                configuration.getSampleRate(), ConstAV.AudioParams.CHANNEL,
                ConstAV.AudioParams.BITS_PER_SAMPLE);
        recorder = new AudioRecord(
                MediaRecorder.AudioSource.MIC, configuration.getSampleRate(),
                ConstAV.AudioParams.CHANNEL, ConstAV.AudioParams.BITS_PER_SAMPLE, bufferSize);
        ...
        sampleDuration = configuration.getSamplePerFrame() * ConstAV.SECOND_IN_US /
                configuration.getSampleRate();
        duration = 0;

        System.out.println("AudioRecorder Runnable thread id " + Thread.currentThread().getId());
    ...
    public void start() {
        ...
        System.out.println("AudioRecorder start beforeloop thread id " + Thread.currentThread().getId());

        recorder.startRecording();
        encoder.start();
        loop();
        started = true;
        System.out.println("AudioRecorder start afterloop thread id " + Thread.currentThread().getId());
    ...
    public void prepareCodec() throws IOException {
        MediaFormat audioFormat = MediaFormat.createAudioFormat(ConstAV.AudioParams.MIME_TYPE,
                ConstAV.AudioParams.SAMPLE_RATE, ConstAV.AudioParams.CHANNEL_COUNT);
        audioFormat.setInteger(MediaFormat.KEY_AAC_PROFILE,
                MediaCodecInfo.CodecProfileLevel.AACObjectLC);
        audioFormat.setInteger(MediaFormat.KEY_CHANNEL_MASK, ConstAV.AudioParams.CHANNEL);
        audioFormat.setInteger(MediaFormat.KEY_BIT_RATE, ConstAV.AudioParams.BIT_RATE);
        audioFormat.setInteger(MediaFormat.KEY_CHANNEL_COUNT, ConstAV.AudioParams.CHANNEL_COUNT);
        audioFormat.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 1024 * 4);
        encoder = MediaCodec.createEncoderByType(ConstAV.AudioParams.MIME_TYPE);
    ...
    private void onEncode(byte[] data, int length) {
        ...
        while (true) {
            final int inputBufferIndex = encoder.dequeueInputBuffer(ConstAV.BUFFER_TIME_OUT);
    ...
    private void drain() {
        System.out.println("AudioRecorder drain thread id " + Thread.currentThread().getId());


        bufferInfo = new MediaCodec.BufferInfo();
        ByteBuffer[] encoderOutputBuffers = encoder.getOutputBuffers();
        int encoderStatus = encoder.dequeueOutputBuffer(bufferInfo, ConstAV.BUFFER_TIME_OUT);
        while (encoderStatus >= 0) {
            ...
            encoderStatus = encoder.dequeueOutputBuffer(bufferInfo, ConstAV.BUFFER_TIME_OUT);
        }
    }
